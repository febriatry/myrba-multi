                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div>
                                ...
                                @if(isset($pppActiveBreakdown) && count($pppActiveBreakdown))
                                    <div class="small text-muted">
                                        @foreach($pppActiveBreakdown as $routerName => $count)
                                            <div>{{ $routerName }}: {{ $count }}</div>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div>
                                ...
                                @if(isset($pppNonActiveBreakdown) && count($pppNonActiveBreakdown))
                                    <div class="small text-muted">
                                        @foreach($pppNonActiveBreakdown as $routerName => $count)
                                            <div>{{ $routerName }}: {{ $count }}</div>
                                        @endforeach
                                    </div>
                                @endif
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div>
                                    ...
                                    <h4 class="my-1 text-primary"><a href="/pelanggans?status=Aktif" class="">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div>
                                    ...
                                    <h4 class="my-1 text-primary"><a href="/pelanggans?status=Non%20Aktif" class="">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div>
                                    ...
                                    <h4 class="my-1 text-primary">
                                        <a href="/pelanggans?from_month={{ date('Y-m') }}&to_month={{ date('Y-m') }}" class="">
                                            {{ $newPelanggan }} Data
                                        </a>
                                    </h4>
                                </div>
...
@push('js')
    ...
    <script>
        ...
        fetch("{{ route('dashboard.invoiceStatusMonthly') }}")
            .then(r => r.json())
            .then(({ labels, paid, waiting, unpaid }) => {
                new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: labels,
    ...
    <script>
        ...
        fetch("{{ route('dashboard.financeMonthly') }}")
            .then(r => r.json())
            .then(({ labels, income, expense }) => {
                new Chart(ctx2, {
                    type: 'bar',
                    data: {
                        labels: labels,
    ...
    <script>
        ...
        function initMap() {
            ...
            const map = new google.maps.Map(document.getElementById('map'), { center: defaultCenter, zoom: 5 });
            const bounds = new google.maps.LatLngBounds();
            let hasMarker = false;
            pelangganData.forEach(pelanggan => {
                const lat = parseFloat(pelanggan.latitude);
                const lng = parseFloat(pelanggan.longitude);
                if (!isFinite(lat) || !isFinite(lng)) return;
                const pos = { lat, lng };
                const marker = new google.maps.Marker({ position: pos, map, title: pelanggan.nama });
                const infowindow = new google.maps.InfoWindow({ content: `<strong>${pelanggan.nama}</strong><br>Lat: ${lat}<br>Lng: ${lng}` });
