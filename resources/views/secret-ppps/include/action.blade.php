    ...
    <div class="dropdown">
        ...
        <ul class="dropdown-menu">
            @can('secret ppp enable')
                <li>
                    <form action="{{ route('secret-ppps.enable', ['id' => $model['.id'], 'router_id' => $model['router_id'] ?? null]) }}" method="post" class="d-inline"
            ...
            @can('secret ppp disable')
                <li>
                    <form action="{{ route('secret-ppps.disable', ['id' => $model['.id'], 'name' => $model['name'], 'router_id' => $model['router_id'] ?? null]) }}"
            ...
            @can('secret ppp delete')
                <li>
                    <form
                        action="{{ route('secret-ppps.deleteSecret', ['id' => $model['.id'], 'name' => $model['name'], 'router_id' => $model['router_id'] ?? null]) }}"
