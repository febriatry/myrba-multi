<div class="row mb-2">
    <div class="col-md-6">
        <div class="form-group">
            ...
            <select class="form-select @error('router_id') is-invalid @enderror" name="router_id" id="router_id" required>
                <option value="" selected disabled>-- {{ __('Pilih Router') }} --</option>
                @foreach (($routers ?? []) as $router)
                    <option value="{{ $router->id }}"
                        {{ (string) ($routerId ?? old('router_id')) === (string) $router->id ? 'selected' : '' }}>
