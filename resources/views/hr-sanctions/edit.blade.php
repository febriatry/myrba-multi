...
@section('content')
    <div class="page-heading">
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-8 order-md-1 order-last">
                    <h3>{{ __('Edit Sanksi') }}</h3>
                    <p class="text-subtitle text-muted">{{ $row->user_name }} ({{ $row->user_email }})</p>
                </div>
                <x-breadcrumb>
                    <li class="breadcrumb-item"><a href="/dashboard">{{ __('Dashboard') }}</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('hr-sanctions.index', ['date' => $row->date]) }}">{{ __('Potongan Sanksi') }}</a></li>
        ...
        <section class="section">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('hr-sanctions.update', $row->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('hr-sanctions.index', ['date' => $row->date]) }}" class="btn btn-light">{{ __('Kembali') }}</a>
                        </div>
                    </form>
                </div>
            </div>
