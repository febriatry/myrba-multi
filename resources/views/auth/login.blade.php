...
@section('content')
    <div class="row h-100">
                ...
                <form method="POST" action="{{ route('login') }}">
                    ...
                    <div class="form-check form-check-lg d-flex align-items-end">
                        <input class="form-check-input me-2" type="checkbox" value="" name="remember" id="remember"
                            {{ (old('remember') || ($isWebViewApp ?? false)) ? 'checked' : '' }}>
        ...
        <div class="col-lg-6 d-none d-lg-block">
            ...
            @endphp
            <div id="auth-right" class="d-flex flex-column align-items-center justify-content-center text-center p-4">
                @if ($settingWeb && $settingWeb->logo)
                    <img src="{{ asset('storage/uploads/logos/' . $settingWeb->logo) }}" alt="Logo Perusahaan"
                         style="max-height: 120px; width: auto; border-radius: 8px; margin-bottom: 16px;">
                @endif
                <h2 class="mb-2">Myrba Billing System</h2>
                <p class="text-muted mb-4">{{ $settingWeb->nama_perusahaan ?? 'Perusahaan' }}</p>
                <div class="mt-auto w-100">
                    <hr>
                    <div class="small text-muted">{{ $settingWeb->nama_perusahaan ?? 'Perusahaan' }}</div>
                </div>
            </div>
        </div>
    </div>
