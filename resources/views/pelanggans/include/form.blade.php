<div class="row mb-2">
    ...
    <div class="col-md-6">
        <div class="mb-3">
            <label class="form-label" for="kode_referal">{{ __('Kode Referal (Opsional)') }}</label>
            <select class="form-control @error('kode_referal') is-invalid @enderror" name="kode_referal"
                id="kode_referal"></select>
            @error('kode_referal')
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-check form-switch mb-3">
                    <input class="form-check-input" type="checkbox" id="generate_tagihan" name="generate_tagihan"
                        value="1" checked>
                    <label class="form-check-label" for="generate_tagihan">
                        Buat Tagihan Otomatis untuk Bulan Pendaftaran
                    </label>
                </div>
            </div>
        </div>
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            $('#kode_referal').select2({
                placeholder: "Cari No Layanan atau Nama Pelanggan",
                allowClear: true,
                ajax: {
                    url: "{{ route('api.search_pelanggan') }}", // Kita akan buat route ini
                    dataType: 'json',
                    delay: 250,
