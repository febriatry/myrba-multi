...
class TagihanController extends Controller
{
    ...
    public function validasiTagihan(Request $request)
    {
        ...
        try {
            ...
            if ($isSetorScheme) {
                ...
                return response()->json(['message' => 'Tagihan berhasil divalidasi, menunggu setor.']);
            }
    ...
    public function publicTagihan(Request $request)
    {
        $tenantId = (int) $request->query('tid', 0);
        $noTagihan = trim((string) $request->query('no_tagihan', ''));
        $noLayanan = trim((string) $request->query('no_layanan', ''));
        if ($tenantId === 0 && $noLayanan !== '') {
        ...
        $result = null;
        if ($tenantId > 0 && ($noTagihan !== '' || $noLayanan !== '')) {
            $query = DB::table('tagihans')
                ...
                ->select(
                    ...
                    'packages.nama_paket as paket_layanan_nama'
                )
                ->where('tagihans.tenant_id', $tenantId);
            if ($noTagihan !== '') {
                $query->where('tagihans.no_tagihan', $noTagihan);
            } else {
                $query->where('pelanggans.no_layanan', $noLayanan);
            }
            $result = $query->orderByDesc('tagihans.tanggal_create_tagihan')->limit(20)->get();
        }

        return view('public.tagihan', [
            'tenantId' => $tenantId,
            'noTagihan' => $noTagihan,
            'noLayanan' => $noLayanan !== '' ? formatNoLayananTenant($noLayanan, $tenantId) : '',
            'rows' => $result,
        ]);
    }
}
